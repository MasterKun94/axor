...
import io.masterkun.kactor.commons.task.DependencyTask;
import io.masterkun.kactor.commons.task.DependencyTaskRegistry;
import io.masterkun.kactor.commons.task.DependencyTaskRegistryRunner;
...
import io.masterkun.kactor.runtime.HasMeter;
...
public class ActorSystemImpl implements ActorSystem, HasMeter {
    ...
    public ActorSystemImpl(String name,
        ...
        if (streamServer == null) {
            ...
            config = ConfigFactory.parseString("kactor.network.bind.port=" + streamServer.bindPort())
                    .withFallback(config);
        }
        ...
        this.actorConfig = ConfigMapper.map(config.getConfig("kactor"), ActorConfig.class);
    ...
    public void register(MeterRegistry registry, String... tags) {
        Gauge.builder("kactor.active.streams.in", remoteActorCache, c -> c.getAll()
                ...
                .register(registry);
        Gauge.builder("kactor.active.streams.out", remoteActorCache, c -> c.getAll()
                ...
                .tag("actor_type", "remote")
                .register(registry);
        Gauge.builder("kactor.active.streams.in", localActorCache, c -> c.values().stream()
                ...
                .register(registry);
        Gauge.builder("kactor.active.streams.out", localActorCache, c -> c.values().stream()
