    ...
    private Object getValue(Config config, String key, TypeRef type) {
        ...
        if (!config.hasPath(key)) {
            if (type.nullable() && !type.parentType().isPrimitive()) {
                ...
                throw new ConfigParseException(key + " not found");
            }
        }
        if (type.nullable() && !config.hasPath(key)) {
            if (type.parentType().isPrimitive()) {
                throw new IllegalArgumentException(key + " is a primitive type and config value is null");
            } else {
                return null;
            }
        }
        if (clazz.isRecord()) {
            ...
            if (type.paramTypes().length != 1) {
                throw new IllegalArgumentException(key + " is a list type, but " + type.paramTypes().length + " type parameters");
            }
            Class<?> elemType = type.paramTypes()[0];
            if (elemType.equals(Void.class)) {
                throw new IllegalArgumentException("elemType should not be null");
            }
        ...
        throw new IllegalArgumentException("illegal type: " + clazz + " for key: " + key);
    }
}
