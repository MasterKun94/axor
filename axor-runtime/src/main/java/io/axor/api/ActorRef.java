...
/**
 * Represents a reference to an actor, which can be used to send messages to the actor. The type
 * parameter T represents the type of message that this actor can receive.
 *
...
public sealed interface ActorRef<T> permits ActorRefRich {
    /**
     * Returns a special {@code ActorRef} that represents the absence of a sender. This is typically
     * used when there is no specific sender for a message, such as in the case of system messages.
     *
     * @param <T> the type of message that the actor can receive
     ...
     *
     * @return the {@code MsgType<? super T>} representing the type of messages this actor can
     * process
     */
     ...
     * This method uses {@code ActorRef.noSender()} as the default sender, which represents the
     * absence of a sender.
     *
    ...
    /**
     * Determines whether the actor referenced by this {@code ActorRef} is local to the current
     * node.
     *
     ...
     *
     * @return {@code true} if this {@code ActorRef} is the special "no sender" reference,
    ...
    /**
     * Attempts to cast this {@code ActorRef} to a new type, provided that the new type is supported
     * by the current message type.
     *
     * @param <P>  the target message type of the actor reference
     * @param type the {@code MsgType<P>} representing the new message type to which the actor
     *             reference should be cast
     * @return an {@code ActorRef<P>} if the cast is successful
     * @throws IllegalArgumentException if the new message type is not supported by the current
     *                                  message type
     */
